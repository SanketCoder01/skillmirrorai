    ...
    try {
      ...
      if (analysisData?.results_json) {
        ...
        const parsedData: ResumeData = {
          resumeScore: results.matchScore || analysisData.match_score || 0,
          atsScore: results.ATSScore || results.atsScore || 0,
          totalSkills: (results.coreSkills?.length || 0) + (results.softSkills?.length || 0),
          experienceYears: results.experience?.totalYears || 0,
          skills: [...(results.coreSkills || []), ...(results.softSkills || [])],
          skillCategories: results.skillCategories || {
            ...
            cloud: 0,
            tools: 0,
            softSkills: 100, // Default to soft skills for non-technical resumes
          },
          skillProficiency: results.skillProficiency || results.skill_scores || {},
          ...
          aiInsights: {
            bestFitRole: results.bestCareerDirection || results.aiInsights?.bestFitRole || "",
            bestFitScore: results.aiInsights?.bestFitScore || results.matchScore || 0,
            skillGaps: results.missingSkills || results.aiInsights?.skillGaps || [],
            strongAreas: results.strengths || results.aiInsights?.strongAreas || [],
          },
          jobMatch: {
            score: results.matchScore || 0,
            matchedSkills: results.coreSkills || [],
